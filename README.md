...
支持作者
<img src="https://github.com/xzrkali/Pico-BadUSB/blob/main/%E5%BE%AE%E4%BF%A1%E6%94%AF%E4%BB%98.png" width="200" alt="微信赞赏码" />
硬件接线
组件	Pico 引脚
...
快速开始
安装 CircuitPython：从 circuitpython.org 下载对应 Pico 的固件，按住 BOOTSEL 键将 .uf2 文件拖入 RPI-RP2 盘。

下载本项目：将 code.py, boot.py 以及 lib 文件夹复制到 CIRCUITPY 盘的根目录。

准备 SD 卡：格式化为 FAT32，在根目录创建 payloads 文件夹，将你的脚本（.txt 文件）放入其中。

接线并重启：按接线图连接硬件，重启 Pico 即可看到 OLED 菜单。

使用：通过上/下键选择脚本，按执行键运行。脚本格式见下文。
...
项目链接
GitHub: https://github.com/xzrkali/Pico-BadUSB
